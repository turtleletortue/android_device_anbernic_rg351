...
DATE=`date +%Y%m%d`
IMGNAME=$LINEAGEVERSION-$DATE-odroidgo3.img
IMGSIZE=7
OUTDIR=${ANDROID_PRODUCT_OUT:="../../../out/target/product/odroidgo3"}
...
if [ -f $IMGNAME ]; then
	...
	sync
	cp $OUTDIR/obj/KERNEL_OBJ/arch/arm64/boot/Image sdcard/BOOT
	cp $OUTDIR/obj/KERNEL_OBJ/arch/arm64/boot/dts/rockchip/rk3326-odroidgo3-linux.dtb sdcard/BOOT	
	cp BOOT/* sdcard/BOOT
